...
\begin{itemize}
    \item Implement prioritized replay buffers that adaptively sample high-information positions more frequently
    \item Design non-iid sampling strategies that break from uniform distribution assumptions and create non-stationary training distributions
    ...
    \item Progress from simple heuristics to sophisticated information-theoretic measures within the prioritized replay framework
...
\begin{itemize}
    ...
    \item Understand information-theoretic measures in the context of neural network training
    \item Review existing approaches to sample-efficient training and curriculum learning
\end{itemize}
...
\begin{tikzpicture}[x=0.5cm, y=0.5cm]
    % Timeline axis
    \draw[->] (0,0) -- (24,0) node[right] {Weeks (Term 1-3)};
    ...
    % Tasks
    \node[left] at (0,1) {Literature Review};
    \draw[fill=blue!30] (0,0.8) rectangle (4,1.2);

    \node[left] at (0,2) {Data Preparation};
    \draw[fill=green!30] (2,1.8) rectangle (8,2.2);

    \node[left] at (0,3) {Replay Buffer Implementation};
    \draw[fill=yellow!30] (6,2.8) rectangle (12,3.2);

    \node[left] at (0,4) {Sampling Methods};
    \draw[fill=red!30] (10,3.8) rectangle (16,4.2);

    \node[left] at (0,5) {Training Integration};
    \draw[fill=purple!30] (14,4.8) rectangle (20,5.2);

    \node[left] at (0,6) {Final Report};
    \draw[fill=orange!30] (18,5.8) rectangle (24,6.2);

    % Legend
    \node[below right] at (0,0) {Legend:};
    \draw[fill=blue!30] (2,-0.5) rectangle (3,-0.3) node[right] {Literature};
    \draw[fill=green!30] (2,-0.8) rectangle (3,-0.6) node[right] {Data};
    \draw[fill=yellow!30] (2,-1.1) rectangle (3,-0.9) node[right] {Replay Buffer};
    \draw[fill=red!30] (2,-1.4) rectangle (3,-1.2) node[right] {Sampling};
    \draw[fill=purple!30] (2,-1.7) rectangle (3,-1.5) node[right] {Training};
    \draw[fill=orange!30] (2,-2.0) rectangle (3,-1.8) node[right] {Report};
\end{tikzpicture}
...
\begin{enumerate}
    \item Kingma, D.P., Welling, M. ``Auto-Encoding Variational Bayes.'' ICLR (2014).
    \item Rezende, D., Mohamed, S. ``Variational Inference with Normalizing Flows.'' ICML (2015).
    \item Klein, D. ``Neural Networks for Chess.'' arXiv:2209.01506 (2022).
    \item Bengio, Y., et al. ``Curriculum Learning.'' Proceedings of the 26th International Conference on Machine Learning (2009).
    \item Lake, B.M., et al. ``Building Machines That Learn and Think Like People.'' Behavioral and Brain Sciences 40, e253 (2017).
    \item Silver, D., et al. ``Mastering Chess and Shogi by Self-Play with a General Reinforcement Learning Algorithm.'' arXiv:1712.01815 (2017).
    \item Ruoss, A., et al. ``Amortized Planning with Large-Scale Transformers: A Case Study on Chess.'' NeurIPS (2024). Available at: \url{https://github.com/google-deepmind/searchless_chess}
\end{enumerate}
