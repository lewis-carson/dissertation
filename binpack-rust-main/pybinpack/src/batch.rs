...
impl HalfKPSparse {
    ...
    const NUM_PLANES: usize = Self::NUM_SQ * Self::NUM_PT + 1;
    ...
    fn fill_features(
        ...
        while pieces != 0 && count < indices.len() {
            ...
            if let Some(plane_index) = Self::piece_plane_index(piece, color) {
                let feature = 1
                    + Self::orient_square(color, square)
                    + plane_index * Self::NUM_SQ
                    + king_bucket * Self::NUM_PLANES;
    ...
    fn piece_plane_index(piece: Piece, color: Color) -> Option<usize> {
        ...
        Some(base * 2 + usize::from(piece.color() != color))
    }
